...
router.post('/create', async (req, res, next) => {
  try {
    const { amount, currency = 'INR', description, customerId, method = 'card' } = req.body;
    ...
    const payment = await hs.createPayment({ amount, currency, description, customerId, metadata: { recommended_gateway: routing.recommended } });
...
router.post('/:id/confirm', async (req, res, next) => {
  try {
    ...
    const confirmed = await hs.confirmPayment({ paymentId: req.params.id, paymentMethod, paymentMethodData });
    res.json(confirmed);
  } catch (err) { next(err); }
});
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const payments = await hs.listPayments({ limit: parseInt(limit), offset: parseInt(offset) });
    res.json(payments);
  } catch (err) { next(err); }
});
...
router.post('/:id/refund', async (req, res, next) => {
  try {
    ...
    const refund = await hs.createRefund({ paymentId: req.params.id, amount, reason });
    res.json(refund);
  } catch (err) { next(err); }
});
