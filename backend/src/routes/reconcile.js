...
router.get("/run", async (req, res, next) => {
  try {
    ...
    try {
      ...
      txns = (real.data || real.payments || []).filter(
        (t) => t.status === "succeeded",
      );
    } catch (_) {}
    if (txns.length < 10) {
      txns = generateMockTransactions(50).filter(
        (t) => t.status === "succeeded",
      );
    }
...
router.post("/dispute-letter", (req, res) => {
  const { paymentId, amount, gateway, createdAt, type, aiExplanation } =
    req.body;
