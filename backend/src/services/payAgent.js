...
const AGENT_TOOLS = [
  {
    ...
    description:
      "Check for payment anomalies — failure spikes, refund patterns, duplicates. Returns detected anomalies with severity.",
    input_schema: {
      type: "object",
      properties: {
        timeRange: {
          type: "string",
          description:
            "Time range to check: 'last_24h' | 'last_6h' | 'last_1h'",
          default: "last_24h",
        },
        gateway: {
          type: "string",
          description: "Specific gateway to check (optional)",
        },
      },
    },
  },
  {
    name: "route_payment",
    description:
      "Get optimal gateway recommendation for a payment, or disable/enable a gateway.",
    input_schema: {
      type: "object",
      properties: {
        action: {
          type: "string",
          description: "'predict' | 'disable' | 'enable' | 'status'",
          default: "status",
        },
        gateway: {
          type: "string",
          description: "Gateway name for disable/enable actions",
        },
        amount: {
          type: "number",
          description: "Payment amount in paise (for predict)",
        },
        method: {
          type: "string",
          description: "Payment method: 'upi' | 'card' | 'wallet'",
        },
      },
    },
  },
  {
    name: "reconcile",
    description:
      "Run reconciliation to find mismatches between gateway and bank records.",
    input_schema: {
      type: "object",
      properties: {
        dateRange: {
        ...
        generateReports: {
          type: "boolean",
          description: "Whether to generate dispute letters",
          default: false,
        },
      },
    },
  },
  {
    ...
    description:
      "Generate a checkout UI config from a brand description using AI.",
    input_schema: {
      type: "object",
      properties: {
        brandDescription: {
          type: "string",
          description:
            'Text description of the brand, e.g. "minimalist luxury coffee brand"',
        },
  ...
  switch (toolName) {
    case "check_anomalies": {
      ...
      return {
        ...
        criticalCount: allAlerts.filter((a) => a.severity === "CRITICAL")
          .length,
        heatmapPeaks: results.heatmap
          .filter((h) => h.rate > 0.2)
          .map((h) => `${h.hour}:00 (${Math.round(h.rate * 100)}% failure)`),
      };
    }
    ...
    case "route_payment": {
      ...
      if (action === "disable" && gateway) {
        ...
        return {
          disabled: gateway,
          rerouted: rerouted.recommended,
          confidence: rerouted.confidence,
        };
      }
    ...
    case "reconcile": {
      const mockTxns = generateMockTransactions(50).filter(
    ...
    case "generate_checkout": {
      ...
      const theme =
        desc.includes("luxury") || desc.includes("premium")
          ? {
              primary: "#1a0a00",
              accent: "#f5b800",
          ...
          : desc.includes("dark") || desc.includes("tech")
            ? {
                ...
                accent: "#00f5a0",
                font: "IBM Plex Mono",
                payOrder: ["card", "upi", "wallet"],
              }
            : {
                primary: "#ffffff",
                accent: "#4d9fff",
                font: "Syne",
                payOrder: ["upi", "card", "wallet"],
              };
      return {
        merchantName,
        theme,
        embedCode: `<script src="https://payos.dev/sdk.js" data-merchant="${merchantName.toLowerCase().replace(/\s/g, "-")}" data-theme='${JSON.stringify(theme)}'></script>`,
      };
    }
...
const runAgentDemo = async (userMessage) => {
  ...
  if (
    ...
    return {
      ...
      toolCallsMade: [
        {
          tool: "route_payment",
          input: { action: "disable", gateway: "payu" },
        },
      ],
    };
  }
  if (
    ...
    return {
      ...
      toolCallsMade: [
        {
          tool: "reconcile",
          input: { dateRange: "last_7_days", generateReports: true },
        },
      ],
    };
  }
  if (
    ...
    return {
      ...
      toolCallsMade: [
        { tool: "generate_checkout", input: { brandDescription: msg } },
      ],
    };
  }
  return {
