...
const matchTransaction = (gatewayTxn, bankEntries) => {
  ...
  const match = bankEntries.find(
    (entry) =>
      ...
      Math.abs(new Date(entry.date) - normalized.createdAt) <
        3 * 24 * 60 * 60 * 1000, // within 3 days (T+2)
  );
  return match ?? null;
};
...
const classifyMismatch = (gatewayTxn, bankEntry) => {
  if (!bankEntry) {
    const daysSince =
      (Date.now() - new Date(gatewayTxn.created || gatewayTxn.created_at)) /
      (1000 * 60 * 60 * 24);
...
const mismatchExplanations = {
  ...
  UNKNOWN: (_txn) => `Unclassified mismatch. Manual review required.`,
};
  ...
  const results = gatewayTxns
    ...
    .map((txn) => {
      ...
      return {
        ...
        disputeLetter:
          !bankMatch && type === "MISSING_SETTLEMENT"
            ? generateDisputeLetter({
                paymentId: txn.payment_id || txn.id,
                amount: txn.amount,
                gateway: txn.connector || txn.gateway,
                createdAt: txn.created || txn.created_at,
                type,
                aiExplanation: mismatchExplanations[type]?.(txn),
              })
            : null,
      };
    });
