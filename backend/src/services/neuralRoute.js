...
const buildTxnContext = (txn) => ({
  ...
  binMatch: txn.cardNumber ? BIN_ROUTING_MAP[txn.cardNumber.substring(0, 6)] : null
});
...
const calculateRuleScore = (ctx, gateway, health) => {
  ...
  if (ctx.isUpi && gateway === 'razorpay') score += 0.10;
  if (ctx.isHighValueCard && ctx.isLateNight && gateway === 'stripe') score += 0.08;
  if (ctx.amount > 500000 && gateway === 'cashfree') score += 0.05;

  if (health[gateway]?.successRate < 0.60) score -= 0.50;
...
const predictGateway = (txn, currentHealthState) => {
  ...
  return {
    ...
    fallbacks: fallbacks.map(f => ({ gateway: f.gateway, confidence: f.confidence })),
    allGateways: ranked,
  };
};
